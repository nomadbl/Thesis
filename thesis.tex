...
\author{Lior Blech}
\author{Nilanjan Sircar}
\author{Jacob Sonnenschein}
...
The SYM theory lives on the boundary of AdS while the string theory lives in the bulk. The perturbative description is valid for small $g_{YM}$ while the supergravity description is valid for large radii, hence large $\lambda$ and large $g_{YM}$. In this sense it is also a weak-strong coupling correspondence. A consequence of the correspondence must be that correlation functions in the gauge theory can be computed in the string theory by using the gauge theory operator insertions as the boundary conditions for the string theory computations. The correspondence in the generating functional is:
\begin{equation}
Z_{SYM}\left(\Phi_0\right)=< e^{\int d^4x\Phi_0\left(x\right)Q\left(x\right)}>
\sim Z_{stringy}\left(\Phi_0\right)=\int\limits_{\Phi\rightarrow\Phi_0}\mathcal{D}\Phi e^{-S\left[\Phi\right]}
\end{equation}
Where $\Phi_0\left(x\right)$ is the source of the boundary field $Q\left(x\right)$.
...
In the standard model, the known relations,
\begin{subequations}
\begin{align}
&Y=B+S+C+B'+T\\
&Q=I_3+\frac{1}{2}Y
\end{align}
\end{subequations}
relate the Hypercharge ,Stranegeness $S=-(n_s-n_{\bar{s}})$, Charm $C=(n_c-n_{\bar{c}})$, Bottomness $B'=-(n_b-n_{\bar{b}})$, Topness $T=(n_t-n_{\bar{t}})$, Baryon number ($\frac{1}{3}$ for quarks) and the Isospin to the Electric charge of the quarks. In this way the flavors provide another dynamical component to the system which we need to take account of. In our model we take account of the electromagnetic interaction by associating electric charge with the string endpoints. We now present the formulation and analyze the model.
...
\begin{subequations}
\begin{align}
&\phi\left(\vec{x},t\right)=\left[\cfrac{e}{\left(1-\vec{\beta}\cdot\vec{n}\right)R}\right]_{ret}\\
...
\draw[dashed,thick,blue] (P)--(O)--node[sloped,above]{$l_2$}(PP) pic[draw=green,fill=green,<->,angle radius=1.5cm,"$ \Delta\phi $"] {angle= P--O--PP};
\end{tikzpicture}
\caption{The geometry of the system. The points $Q$,$P$ and $P_{ret}$ are the "recieving" particle, the emmiting particle and the position of the emmiting particle at retarded time, respectively. $\Delta\phi$ is the so called retardation angle. $l_1$ and $l_2$ are the respective radii of rotation and $R_{ret}$ is the retarded distance.}
\label{fig:geometry}
\end{figure}

We will see shortly that the key quantities will be the velocities of the quarks $\beta_1$ ,$\beta_2$ and  the retardation angle $\Delta\phi$ defined in figure \ref{fig:geometry}. Looking at figure \ref{fig:geometry} we can use the cosine law to relate the retardation angle with the retarded distance $R_{ret}$:
\begin{equation*}
R_{ret}^2=l_1^2+l_2^2-2l_1l_2\cos\left(\pi-\Delta\phi\right)
\end{equation*} 
...
\item The time it takes the emmiting particle to get from $P_{ret}$ to $P$. $\omega l_2 \left(t-t_{ret}\right)=l_2\Delta\phi$
\end{itemize}
Where $\omega$ is the rate of rotation of both particles. The conclusion being that $\frac{\Delta\phi}{\omega}=R_{ret}$. Combining this with the cosine law result gives:
\begin{equation}
\label{eq:retardation}
\Delta\phi^{2}=\beta_{1}^{2}+\beta_{2}^{2}+2\beta_{1}\beta_{2}cos\left(\Delta\phi\right)
\end{equation}

Here $\beta_i=\omega l_i$.This condition relates the “retardation angle” $\Delta\phi$ and the velocities of the string end points. This equation is solved numerically, giving implicitly the function $\Delta\phi=f\left(\beta_{1}\right)$.
...
\label{eq:fields}
\begin{align}
&A^{0}\left(l_{1}\right)=\frac{q\omega}{\beta_{1}\beta_{2}\sin(\Delta\phi)+\Delta\phi}\\
&A^{\theta}\left(l_{1}\right)=-\frac{\beta_{2}q\omega\cos(\Delta\phi)}{\beta_{1}\beta_{2}\sin(\Delta\phi)+\Delta\phi}\\
&\vec{E}=\left\{ \begin{array}{c}
-\frac{q\omega^{2}\left(\beta_{1}\left(\beta_{2}^{2}\cos(2\Delta\phi)+\beta_{2}^{2}-2\right)+2\beta_{2}\cos(\Delta\phi)\left(\beta_{2}^{2}-1\right)-2\Delta\phi\beta_{2}\sin(\Delta\phi)\right)}{2\left(\Delta\phi+\beta_{1}\beta_{2}\sin(\Delta\phi)\right)^{3}}\\
\frac{q\beta_{2}\omega^{2}\left(\sin(\Delta\phi)\left(\beta_{1}^{2}+\beta_{2}^{2}+\beta_{1}\beta_{2}\cos(\Delta\phi)-1\right)+\Delta\phi\left(\cos(\Delta\phi)+\beta_{1}\beta_{2}\right)\right)}{\left(\Delta\phi+\beta_{1}\beta_{2}\sin(\Delta\phi)\right)^{3}}\\ 0 \end{array}\right\}\\
&\vec{B}=\left\{ \begin{array}{c}
0\\
0\\
\frac{q\beta_{2}\omega^{2}\left(\Delta\phi\left(\beta_{1}^{2}\beta_{2}+\beta_{1}\cos(\Delta\phi)\left(\beta_{2}^{2}+1\right)+\beta_{2}\right)+\beta_{1}\sin(\Delta\phi)\left(\beta_{1}^{2}+2\beta_{1}\beta_{2}\cos(\Delta\phi)+\beta_{2}^{2}\right)\right)}{\Delta\phi\left(\Delta\phi+\beta_{1}\beta_{2}\sin(\Delta\phi)\right)^{3}}
\end{array}\right\}
\end{align}
...
&=T\frac{sin^{-1}(\beta_{1})}{\omega}+T\frac{sin^{-1}\left(\beta_{2}\right)}{\omega}+\cfrac{m_{1}}{\sqrt{1-\beta_{1}^{2}}}+\cfrac{m_{2}}{\sqrt{1-\beta_{2}^{2}}}+\frac{q_{1}q_{2}\omega}{\beta_{1}\beta_{2}\sin(\Delta\phi)+\Delta\phi}
\end{align} 
\FloatBarrier
...
&-q_{1}q_{2}\frac{\beta_{1}\beta_{2}\cos(\Delta\phi)}{\beta_{1}\beta_{2}\sin(\Delta\phi)+\Delta\phi}-q_{1}q_{2}\frac{\beta_{1}\beta_{2}\cos(\Delta\phi)}{\beta_{1}\beta_{2}\sin(\Delta\phi)+\Delta\phi}=\notag\\
&=T\cfrac{-\cfrac{\beta_{1}}{\gamma_{1}}+sin^{-1}\left(\beta_{1}\right)-\cfrac{\beta_{2}}{\gamma_{2}}+sin^{-1}\left(\beta_{2}\right)}{2\omega^{2}}+m_1\cfrac{\gamma_{1}\beta_{1}^{2}}{\omega}+m_2\cfrac{\gamma_{2}\beta_{2}^{2}}{\omega}-q_{1}q_{2}\frac{2\beta_{1}\beta_{2}\cos(\Delta\phi)}{\beta_{1}\beta_{2}\sin(\Delta\phi)+\Delta\phi}
\end{align} 
\FloatBarrier
...
\label{eq:regge}
\begin{align}
&E=\cfrac{T}{\omega}\left(\sin^{-1}(\beta_{1})+\sin^{-1}(\beta_{2})\right)+\gamma_{1}m_{1}+\gamma_{2}m_{2}+\frac{q_{1}q_{2}\omega}{\beta_{1}\beta_{2}\sin(\Delta\phi)+\Delta\phi}-\frac{\pi}{2}\frac{\tilde{a}\omega}{\beta_{1}+\beta_{2}}\\
&J=T\cfrac{-\cfrac{\beta_{1}}{\gamma_{1}}+sin^{-1}\left(\beta_{1}\right)-\cfrac{\beta_{2}}{\gamma_{2}}+sin^{-1}\left(\beta_{2}\right)}{2\omega^{2}}+m_{1}\cfrac{\gamma_{1}\beta_{1}^{2}}{\omega}+m_{2}\cfrac{\gamma_{2}\beta_{2}^{2}}{\omega}-2q_{1}q_{2}\beta_{1}\beta_{2}\frac{cos\left(\Delta\phi\right)}{\beta_{1}\beta_{2}sin\left(\Delta\phi\right)+\Delta\phi}\\
&\Delta\phi^{2}=\beta_{1}^{2}+\beta_{2}^{2}+2\beta_{1}\beta_{2}cos\left(\Delta\phi\right)\\
...
\end{equation*}
\FloatBarrier
\subsection{Taking The Nonrelativistic Limit}
...
We start out from equations \ref{eq:regge}, taking the limit $\beta_1\rightarrow0$. We need to determine the $\beta_1$ dependence of the quantities $\beta_2$,$\Delta\phi$ and $\sin\left(\Delta\phi\right)$. 
...
Using the same method we compute $\Delta\phi$ from equation \ref{eq:retardation}. We put in the solution for $\beta_2$ and get
\begin{equation*}
\Delta\phi=\frac{m_1+m_2}{m_2}\beta_1+O\left(\beta_1^2\right)
\end{equation*}
The solution of $\sin\left(\Delta\phi\right)$ relevant to us is immediate by noting that for $\Delta\phi\rightarrow0$, $\Delta\phi\approx\sin\left(\Delta\phi\right)$.

Mathematica code for the computation of $\Delta\phi$:
...
\FloatBarrier
\subsection{Obtaining Analytic Trajectories}
\label{sec:analytic}
\subsubsection{Relativistic Limit}
The code for obtaining $\beta_2\left(\beta_1\right)$ and $\Delta\phi\left(\beta_1\right)$ (here called $\theta$):
...
\FloatBarrier
\subsection{Mass Difference Code}
