...
Where we have set $\beta_2=0$ so that one of the endpoints is stationary, and $L$ is the string length. Upon doing fits we see that typically the velocity of the light endpoint is such that $\gamma$ is order one. The accuracy of the procedure can be estimated by observing that by the relation
\begin{equation*}
...
\end{equation}
The leading correction to the energy comes from the stringy term and is approximately:
\begin{equation*}
\Delta E \approx TL\frac{m_1}{m_2}\approx (E-m_1-m_2)\frac{m_1}{m_2}
\end{equation*}
In the asymmetric fits we did to the $D$ meson the correction to the energy is then 6\% at the most, so the correction to $E^2$ and the trajectory would be less than 1\%.
...
$a_0$ & 0.97-1.06 & 0-67 \\
$\Phi$  & 0.89-1.09 & 345-425 \\
$\kappa$  & $>$ 0.845 & $>$ 218 \\
$K$ & 0.57-0.86 & 0-228 \\
$D$ & 0.58-1 & 733-923 \\
$D$ & 0.86 & $m_u=150$, $m_c=1594$ \\
%$B$ & 0.48-1 & 2480-2660 \\
$\varUpsilon$  & 0.24-0.4 & 4095-4450 \\
\hline
\end{tabular}
\caption{Meson Fits Summary Table. The fit is symmetric unless endpoint masses indicate different endpoint masses.}
\label{tab:mesonsum}
%\end{table}
...
$\Sigma$ & 0.9-1.5 & 200-609 \\
$\Xi$  & 0.847-1.5 & 459-657 \\
\hline
\end{tabular}
...
The mass difference calculated from the symmetric fit to the $D$ trajectory turns out to be negative which is also not consistent with other calculations. We have tried using parameters from both a symmetric and a non symmetric fit to the $D$ trajectory. The results are displayed in table \ref{tab:ASDMD}. It seems that allowing for asymmetric masses can improve the situation in the quark mass difference calculation but cannot bring the mass difference to a positive value.
...
$K^{*+}$ & 0.86 & 889.11 & $\frac{2}{9}$ & \{-0.27,-0.17\} & 1  \\
$K^{*0}$ & 0.86 & 895.81 & $-\frac{1}{9}$ & \{-0.27,-0.17\} & 1  \\
$\Sigma^+$ & 0.95 & 1189.3 & $\frac{2}{9}$ & $\{-0.19,-0.12\}$ & $\frac{1}{2}$  \\
$\Sigma^-$ & 0.95 & 1197.449 & $\frac{2}{9}$ & $\{-0.19,-0.12\}$ & $\frac{1}{2}$  \\
$\Xi^0$ & 0.94 & 1314.86 & $-\frac{1}{9}$ & $\{-0.18,-0.03\}$ & $\frac{1}{2}$  \\
$\Xi^-$ & 0.94 & 1321.71 & $\frac{2}{9}$ & $\{-0.18,-0.03\}$ & $\frac{1}{2}$  \\
$D^0$ symmetric fit & 0.86 & 1864.84 & $-\frac{4}{9}$ & $\{-0.012, 0\}$ & 0  \\
$D^+$ symmetric fit & 0.86 & 1869.61 & $\frac{2}{9}$ & $\{-0.012, 0\}$ & 0  \\
$D^0$ asymmetric fit & 0.86 & 1864.84 & $-\frac{4}{9}$ & $\{-0.022, -0.012\}$ & 0  \\
$D^+$ asymmetric fit & 0.86 & 1869.61 & $\frac{2}{9}$ & $\{-0.022, -0.012\}$ & 0  \\
\hline
\end{tabular}
...
\begin{figure}[h]
\centering
\includegraphics[scale=1]{figures/MassDifferences2.png}
...
u Mass[MeV] & $\alpha'$[$GeV^{-2}$] & charm Mass[MeV] & $\tilde{a}$ & $m_d-m_u$[MeV] & $\frac{\chi^2_m}{\chi^2_l}$ \\ \hline
...
The resulting model was confronted with PDG data and the model parameters extracted for various hadrons. A new calculation which was attempted is the determination of the u/d mass difference. A mass difference between the u/d quarks exists in the range of $(-15)-10MeV$. The inconsistency is most prevalent when considering hadrons containing more massive quarks such as the \textbf{D} and \textbf{$\kappa$} mesons.
...
In conclusion, we get similar parameters for the Regge trajectory fits and we can determine the u/d quark mass difference. It should be emphasised the mass parameters in our model, as well as their differences, are not constituent quark model masses or current masses. Future research would address determining more precisely the electromagnetic radiation width relative to the strong interaction width, incorporating leptons into the model and incorporating the weak interaction into the model. 
\appendix
\FloatBarrier
\section{Conventions}
