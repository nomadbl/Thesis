...
Using the methods outlined in the previous section, we fit the regge trajectories of our model to PDG data for the hadrons. We assume $m_{1}=m_{2}=m$ and generalise using the relation
\begin{equation}
2m^{3/2}=m_1^{3/2}+m_2^{3/2}
\end{equation}
that holds in the leading order as can be seen from equation \ref{eq:reggeAnalyticLight} and the fits done in \cite{Sonnenschein14} which demonstrate this. Also we take $\alpha_{e}=\frac{1}{137}$.
