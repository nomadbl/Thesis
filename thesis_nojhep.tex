...
Before going into the specifics of our model, it is worth mentioning another approach available for probing the low energy dynamics of QCD. The approach of non-relativistic potential models postulates that the binding force inaccessible to perturbation theory can be written as a non-relativistic potential, commonly:
\begin{equation}
...
Electromagnetic mass differences for hadrons are known for states with $J_{orbital}=0$, therefore we first analyse the case of $\beta=0$. In the stationary string (equation \ref{eq:nonrelativistic2}):
\begin{align}
